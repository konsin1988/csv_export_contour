CREATE PROCEDURE asukrtinform_3.export_csv()
BEGIN
WITH ra_agg AS (
    SELECT
        c.id AS companyId,
    ...
    FROM `asukrtinform_3`.companies c
    JOIN `asukrtinform_3`.formDataRA ra ON c.id = ra.companyId
...
oz_agg AS (
    ...
    FROM `asukrtinform_3`.companies c
    JOIN `asukrtinform_3`.formDataOZ oz ON c.id = oz.companyId
...
rcp_agg AS (
    ...
    FROM `asukrtinform_3`.companies c
    JOIN `asukrtinform_3`.formDataRCP rcp ON c.id = rcp.companyId
...
JOIN asukrtinform_3.company_file cf ON cf.inn = m.inn
